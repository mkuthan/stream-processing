...
class BeamUserSessionsTest extends PipelineSpec with TimestampedMatchers {
  ...
  "Short visit" should "be aggregated into single session" in runWithContext { sc =>
    ...
    results.withTimestamp should containSingleValueAtWindowTime(
      "00:11:00", ("jack", Iterable("open app", "close app"))
    )
  }
  ...
  "Short visit from two clients" should "be aggregated into two simultaneous sessions" in runWithContext { sc =>
    ...
    results.withTimestamp should containInAnyOrderAtWindowTime(Seq(
      ("00:11:00", ("jack", Iterable("open app", "close app"))),
      ("00:11:30", ("ben", Iterable("open app", "close app")))
    ))
  }
  ...
  "Long but continuous visit" should "be aggregated into single sessions" in runWithContext { sc =>
    ...
    results.withTimestamp should containSingleValueAtWindowTime(
      "00:23:10", ("jack", Iterable("open app", "search product", "open product", "add to cart", "checkout", "close app"))
    )
  }

  "Long intermittent visit" should "be aggregated into two sessions" in runWithContext { sc =>
    val userActions = testStreamOf[(User, Action)]
      .addElementsAt("00:00:00", ("jack", "open app"))
      .addElementsAt("00:01:00", ("jack", "search product"))
      .addElementsAt("00:01:30", ("jack", "open product"))
    ...
    results.withTimestamp should containInAnyOrderAtWindowTime(Seq(
      ("00:13:00", ("jack", Iterable("open app", "search product", "open product", "add to cart"))),
  ...
  "Foo" should "bar" in runWithContext { sc =>
    val userActions = testStreamOf[(User, Action)]
      .addElementsAt("00:00:00", ("jack", "open app"))
      .addElementsAt("00:01:00", ("jack", "search product"))
      .addElementsAt("00:01:30", ("jack", "open product"))
    ...
    results.withPaneInfo.withTimestamp.debug()
