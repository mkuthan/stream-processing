...
object PubSubClient extends GcpProjectId with LazyLogging {
  ...
  def generateTopicName(): String =
    s"projects/$projectId/topics/test-topic-temp-${RandomString.randomString()}"

  def generateSubscriptionName(): String =
    s"projects/$projectId/subscriptions/test-subscription-temp-${RandomString.randomString()}"
